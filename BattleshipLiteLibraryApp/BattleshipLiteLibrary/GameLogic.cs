...
public static class GameLogic
{
    public static int GetShotCount(PlayerInfoModel player)
    {
        ...
        foreach (var shot in PlayerInfoModel.ShotGrid)
        {
            shotCount += 1;
        }
    ...
    {
        bool isAHit = true; // not DRY - refactor

        foreach (var ship in PlayerInfoModel.ShipLocations)
        {
    ...
    public static bool PlaceShip(PlayerInfoModel model, string? location)
    {
        ...
        {
            PlayerInfoModel.ShipLocations.Add(new GridSpotModel
            {
    ...
    private static bool ValidateGridLocation(PlayerInfoModel model, string row, int column)
    {
        ...
        foreach (var ship in PlayerInfoModel.ShotGrid)
        {
            if (ship.SpotLetter == row.ToUpper() && ship.SpotNumber == column)
            {
    ...
    public static (string row, int column) SplitShotIntoRowAndColumn(string shot)
    {
        ...
        if (shot.Length != 2)
        {
            throw new ArgumentException("\nThis was an invalid shot type.", shot);
        }
    ...
    private static void AddGridSpot(PlayerInfoModel model, string letter, int number)
    {
        ...
        PlayerInfoModel.ShotGrid.Add(spot);
    }
}